...
- Inverse matrices only exist for square matrices that are symmetric and positive definite.

- We do not discuss what it means for a matrix to be symmetric or positive definite because that is beyond the scope of this course. 
- We can learn a little more about matrices at <https://jfrench.github.io/LinearRegression/overview-of-matrix-facts.html>.
...
Thus, our solution for $\hat{a}$ and $\hat{b}$ that minimize $RSS(a, b)$ is $\hat{a} = 3.58$ and $\hat{b} = 0.69$.

We use `abline` function to add the line of best fit to our scatter plot below.

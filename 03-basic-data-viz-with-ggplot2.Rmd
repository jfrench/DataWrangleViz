...
We will use the `penguins` data set in the **palmerpenguins** package [@palmerpenguins] to illustrate some of the ways that **ggplot2** can be used. 
...
The data set includes `r nrow(penguins)` observations of `r ncol(penguins)`. The variables are:
...
We can color the bars for visual clarity by adding the `fill` aesthetic.
...
A *histogram* is used to display the distribution of a continuous `numeric` variable. The range fo the variable is partitioned into classes. The number of observations falling in each class is counted. A histogram draws a bar for each class with height corresponding to the number of observations in that class.
...
A density plot is often preferred to a histogram because it's more flexible, though it often provides similar information. A density plot is essentially a smoothed version of a histogram. It is appropriate for displaying the distribution of continuous `numeric` variables. A density plot indicates the values for which the data is more "dense". More specifically, density plots indicates the values of data you are most likely to observe. A higher density region means you are more likely to observe data in that regoin. There is a statistical definition we will not go into.
...
A weakness of the previous graphic is that that to legend distinguishing the different densities is a bit subtle. The line around the border may not be easy to distinguish for everyone. We could consider changing the `linetype` or we could use both `color` and `linetype`.
...
When a variable is mapped to an aesthetic, **ggplot2** will assign a unique level to the variable. This process is known as *scaling*.
...
A boxplot is a robust display of information about a continuous `numeric` variable. Boxplots are robust because they rely on robust statistics not easily affected by outliers. 
...
Unsurprisingly, the graphics produced by **gplot2** can be greatly customized. We consider some of the basic customizations below.
...
e.g., If we want to compare the number of penguins of each sex within species, it would be better to facet the data by `sex` and display the count of each species.
...
We do not discuss coordinate systems in detail here. The only coordinate system I want to mentioned is the "flipped" coordinate system (`coord_flip`), which flips the x and y axes. This is useful for rating objects in a plot. Consider the boxplots below.


```{r}
ggplot(penguins) + geom_boxplot(aes(y = body_mass_g, fill = species))
ggplot(penguins) + geom_boxplot(aes(y = body_mass_g, fill = species)) + coord_flip()
```

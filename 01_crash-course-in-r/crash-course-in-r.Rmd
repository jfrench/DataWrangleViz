...
## R Studio
R Studio Desktop is a free "front end" for R provided by [R Studio](https://rstudio.com/products/rstudio/download/). R Studio Desktop makes doing data science with R much easier by adding an Integrated Development Environment (IDE) and providing many other features. Currently, you may download R Studio at this [link](https://rstudio.com/products/rstudio/download/). You may need to navigate the R Studio website directly.
...
## Your turn {.action}
The **tidyverse** [www.tidyverse.org](www.tidyverse.org) is an ecosystem of R packages that we will use extensively in this class. Currently, the **tidyverse** is comprised of the following packages:
...
## Data types
R has 6 basic [data types](https://swcarpentry.github.io/r-novice-inflammation/13-supp-data-structures/):
...
In addition to the five listed above, there is also raw which will not be discussed.
...
The name of the function is `myfunction`. To use this function, I need to supply 3 arguments.

A function may or may not return something back that you can store for later use.

To use the function, you simply type `myfunction(arg1, arg2, arg3)` into the R console.

Let's perform an example of a function that returns the standard deviation of a vector `x`.

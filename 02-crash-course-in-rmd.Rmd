...
```{r, include = FALSE}
knitr::write_bib(c("bookdown"), file = "packages_crashrmd.bib")
```
...
by following the link [here](https://www.youtube.com/playlist?list=PLkrJrLs7xfbXJ0F9yc-n8PzvoFv4qmM-N). I have also included direct, relevant video links throughout the file (e.g., immediately after the the relevant heading). An html version of this document can be downloaded to your current working direction (run `getwd()` in the Console to see where this is) by running the following command:
```{r, eval=FALSE}
...
Unless you're using R Markdown for advanced purposes (books, blogs, websites, running the same file with multiple input parameters) then you probably don't need to know a lot about the YAML header at this stage. And if you do need to know about the YAML header in detail, then you probably need a more advanced introduction to R Markdown!
...
You can work with these documents in the same way as the standard R Markdown documents, but with additional cross-referencing capabilities.

The **bookdown** package also provides document types for html books, LaTeX books, and e-books, though we don't discuss there any more. Some of the available document `bookdown::gitbook`, `bookdown::pdf_book`, `bookdown::html_book`
...
- A `csl` file using the Citation Style Language ([https://citationstyles.org/](https://citationstyles.org/)).
...
- The BibTeX website ([https://www.bibtex.com](https://www.bibtex.com)) is an excellent source of information about using BibTeX. 
...
Refer to ([https://www.bibtex.com/g/bibtex-format/](https://www.bibtex.com/g/bibtex-format/)) for additional fields.
...
The `.bib` file should be in the same directory as the document citing the entries. In your YAML header, you must add the following line to link the `.bib` file with your document:
````{verbatim}
...
It is possible to change citation styles, use multiple `.bib` files in one document, have hyperlinks to your references. etc. We do not discuss those options, but refer you to [https://bookdown.org/yihui/bookdown/citations.html](https://bookdown.org/yihui/bookdown/citations.html).
...
The text above adds the label `citations` to the subsection "Citations". To refer to the section, we use the syntax `\@ref(label)`, where `label` is the unique section label. To cite the "Citation" section, we would write `\@ref(citations)`.

Adding the following text in your document:
````{verbatim}
We discuss citations in Section \@ref(citations).
````
will result in:

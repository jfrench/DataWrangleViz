...
<img src="./figures/quarto_logo.png" height="154" />
...
More helpfully, Quarto is the spiritual successor to R Markdown that was built from the ground up to be compatible with R, Python, Julia, and Observable JavaScript. You can use Quarto to create: 
...
To add the "Favorites" page to our website, we open the `_quarto.yml` file and navigate to the `navbar` section where `.qmd` files are listed are a dash. We add a new row with `- favorites.qmd` as in the code below.
...
Quarto provides over 20 themes "out of the box". Some of the
best are:
...
Quarto website colors are specified in hexadecimal format. The website [https://htmlcolorcodes.com/](https://htmlcolorcodes.com/) is a handy tool for determining hexadecimal values to make font or background color changes.
...
- `links`: links you want to provide for the user to click on. These could be links to your GitHub page, email address, Twitch account etc. The links have the following options:
  ...
  - `icon`: the name of one of the standard Bootstrap 5 icons (e.g. "envelope", "github", "twitter", "youtube", etc.).
...
In the example below, we will replace the contents of `index.qmd` with the code below. You will need to add three files to your project directory:

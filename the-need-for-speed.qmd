...
<a href="https://colab.research.google.com/github/jfrench/DataWrangleViz/blob/master/the-need-for-speed.ipynb">
   <img src="https://raw.githubusercontent.com/jfrench/DataWrangleViz/refs/heads/master/qr_codes/qr-need-for-speed.svg" width="300">
</a>
...
```{r}
# see if microbenchmark package is available
...
```
```{r}
# see if pbapply package is available
...
```
```{r}
# see if future package is available
...
```
```{r}
# see if future.apply package is available
...
We will use the `microbenchmark` function from the **microbenchmark** package [@R-microbenchmark] to time various computational approaches below.
...
We use the `microbenchmark` function to compare the timings of `cumsum_loop` and `cumsum`. The `microbenchmark` function will run the provided functions 100 times, by default.

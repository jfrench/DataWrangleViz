...
- `else if` must be used in conjunction with the `if` function and should be used with an `else` statement (otherwise there may be cases that are left out).
...
The `else if` and `else` functions should be one space after the `}` used to indicate the end if the previous `if` or `elseif` statement.

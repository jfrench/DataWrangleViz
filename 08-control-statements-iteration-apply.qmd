...
- The `if` function can be used by itself.
- The `else` statement must be used in conjunction with the `if` function.
- `elseif` must be used in conjunction with the `if` function and should be used with an `else` statement (otherwise there may be cases that are left out).

An `if`  or `elseif` statement is used to perform a sequence of commands when its argument is `TRUE`. 
...
A set of `if`, `elseif`, and `else` statements will having the following structure:

```
if(logical statement) {
  # code to run if logical statement is true
} elseif (another logical statement) {
...
The `elseif` and `else` functions should be one space after the `}` used to indicate the end if the previous `if` or `elseif` statement.
...
There is also a `repeat` iterator that repeatedly runs the same set of code until the `break` function is used.
